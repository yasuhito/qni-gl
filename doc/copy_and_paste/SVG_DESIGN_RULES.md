...
この章のSVGは見た目確認用の参考であり、コピー＆ペースト機能の正本ではない。

現在の動作仕様は `SPEC.md` を正とする。
SVG例に旧仕様のペースト強調表示が含まれる場合でも、実装では `SPEC.md` の仕様を優先する。
...
挿入アンカーは、コピーした分の最も右のステップの1量子ビット目。
...
挿入アンカーは、コピーした量子ゲートのセットの最も右のステップの1量子ビット目。

複数ゲートのペーストの場合、ペーストされる側の最も左のステップの1量子ビット目が、挿入アンカーの右に位置するよう複製される。
...
### 2-9-2. 真ん中のSゲートを挿入アンカーにする

CNOT・S・CNOTのゲートセットを複数選択しコピーした後、真ん中のSゲートを選択し挿入アンカーにした状態。
...
制御線が描画されていても、ゲートはないので空白だが、選択し挿入アンカーとして利用可能。
