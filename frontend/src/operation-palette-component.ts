 ...
 */
export class OperationPaletteComponent extends Container {
  ...
  constructor() {
    ...
    this.rows = new List({
      ...
      vertPadding: OperationPaletteComponent.verticalPadding,
  ...
  private setupOperationSourceEvents(
    operationSource: OperationSourceComponent
  ) {
    operationSource.on(
  ...
  private redraw(): void {
    ...
    const height =
      this.rows.height + OperationPaletteComponent.verticalPadding * 2;

    this.body
      .clear()
      .roundRect(0, 0, width, height, OperationPaletteComponent.cornerRadius)
