...
export class OldGatePalette {
  ...
  get width(): number {
    return (
      ...
      OldGatePalette.gapBetweenGates * (this.horizontalGateCount - 1) +
      OldGatePalette.horizontalPadding * 2
    );
  }
  ...
  get height(): number {
    ...
    return (
      Gate.size * gateRows +
      OldGatePalette.gapBetweenGates * (gateRows - 1) +
      OldGatePalette.verticalPadding * 2
    );
  }
  ...
  addGate(gateClass: typeof Gate, row = 1): Gate {
    ...
    const x =
      this.x +
      OldGatePalette.horizontalPadding +
      (this.gateClasses[row].length - 1) *
        (Gate.size + OldGatePalette.gapBetweenGates);
    const y =
      this.y +
      OldGatePalette.verticalPadding +
      (row - 1) * (Gate.size + OldGatePalette.gapBetweenGates);
    const gateSource = new GateSource(gateClass);
    gateSource.x = x;
    gateSource.y = y;
    this.graphics.addChild(gateSource.view);
  ...
  draw(): void {
    ...
    this.graphics.lineStyle(1, OldGatePalette.borderColor, 1, 0);
    this.graphics.beginFill(OldGatePalette.backgroundColor);
    this.graphics.drawRoundedRect(
      this.x,
      this.y,
      this.width,
      this.height,
      OldGatePalette.cornerRadius
    );
