...
def assert_amplitudes_approx(actual_amplitudes, expected_amplitudes):
    ...
    for key, (expected_real, expected_imag) in expected_amplitudes.items():
        ...
        assert pytest.approx(actual_real) == expected_real, f"Real parts differ for key {
            key}: {actual_real} != {expected_real}"
        assert pytest.approx(actual_imag) == expected_imag, f"Imaginary parts differ for key {
            key}: {actual_imag} != {expected_imag}"
