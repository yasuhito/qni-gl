...
def _map_controlled_gate(op_type: str) -> str | None:
    """
    Maps Qiskit controlled gate names to Qni gate names.
    """
    gate_map = {
...
def convert_qiskit_circuit_to_qni(qc: QuantumCircuit):
    """
    Convert a Qiskit QuantumCircuit to Qni's cols/steps representation.

    Returns:
        tuple[list[list[dict]], list[list[str | None]], int]:
            steps: List of step dictionaries (operations per column)
            cols: List of columns (gate symbols or None)
...
def _build_column_for_layer(layer, qc, num_qubits, pending_reset):
    """
...
def _handle_multi_and_controlled_gates(layer, qc, column, operations_in_step, processed_qubits):
    """
    ...
    for node in layer['graph'].op_nodes():
        ...
        if hasattr(node.op, "num_ctrl_qubits") and node.op.num_ctrl_qubits > 0:
            ...
            for ctrl in controls:
                ...
                operations_in_step.append({"type": "•", "targets": [ctrl], "controls": []})

            for tgt in targets:
                column[tgt] = qni_gate
                processed_qubits.add(tgt)
                operations_in_step.append({"type": qni_gate, "targets": [tgt], "controls": controls})
        ...
        if op_type in ("MCX", "CCX"):
            ...
            operations_in_step.append({"type": "X", "targets": [target], "controls": controls})

            continue

        # 2-qubit controlled gates (CX, CY, etc.)
        qni_gate = _map_controlled_gate(op_type)
        if qni_gate and len(qargs) == 2:
            ctrl, tgt = qargs
            column[ctrl] = "•"
            column[tgt] = qni_gate
            processed_qubits.update([ctrl, tgt])
            operations_in_step.append({"type": "•", "targets": [ctrl], "controls": []})
            operations_in_step.append({"type": qni_gate, "targets": [tgt], "controls": [ctrl]})
            continue
...
def _handle_single_qubit_gates_and_reset(layer, qc, column, operations_in_step, processed_qubits, pending_reset):
    """
...
def _finalize_pending_resets(column, operations_in_step, pending_reset):
    """
...
def cols_to_steps(cols: list[list[str | int | None]]) -> list[list[dict]]:
    """
    Convert Qni's cols representation to a list of step dictionaries for simulation/export.

    Each step is a list of operation dictionaries, with optional "controls" for controlled gates.

    Args:
        cols (list[list[str | int | None]]): Qni column representation.

    Returns:
        list[list[dict]]: List of steps, each step is a list of operation dicts.
    """
    steps = []
    for col in cols:
        ...
        for idx, gate in enumerate(col):
            if gate is None or gate == "•" or gate == 1:
                continue
